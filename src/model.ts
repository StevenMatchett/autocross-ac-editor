...
export function validateLayout(value:unknown):Layout {
 ...
 if(!v || v.version!==1 || typeof v.name!=='string' || v.name.length>120 || !Number.isInteger(v.columns) || !Number.isInteger(v.rows) || v.columns<4 || v.columns>80 || v.rows<4 || v.rows>80 || !Array.isArray(v.items) || v.items.length>10000) throw Error('This is not a supported Padwork layout.');
