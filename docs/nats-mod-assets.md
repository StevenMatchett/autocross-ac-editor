...
The editor uses an 84 × 74 pad bounding rectangle around the main lot. The top-down image is rendered from that same model, with north toward decreasing Z. The grid is an optional spacing guide and does not assert joint alignment. Loading the 2026 example rotates its objects and headings 180° around the center of the traced 39 × 56 pad rectangle, then translates by +274.32 m X and +68.58 m Z (36 and 9 pads). This shifts the course 150 feet west toward the lot center from its initial placement. No scaling is applied; the 75-foot calibration and relative spacing are preserved. This is an approximate east-apron placement, not surveyed joint alignment. The app always uses Lincoln. The original flat example coordinates remain in the source fixture for calibration; loading transforms a fresh copy. Legacy flat Nationals layouts receive the same rotation and placement; other flat saved layouts are centered with a single translation in whole-pad increments; files too large for the venue are rejected instead of clipped.
